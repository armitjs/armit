...
import { createThreadPool, type WrapWorkerType } from '../index.js';
import majorVersion from '../utils/major-node-version.js';
import { type WorkerThread } from '../worker-thread.js';
import type BasicWorker from './workers/basic.js';
import { type WorkerWithCallback } from './workers/callback.js';
import { type ValidWorkerClass } from './workers/class.js';
import { type WorkerWithEmitter } from './workers/eventemitter.js';
import { type ValidWorkerModule } from './workers/module.js';
import { type ChainWorkerClass } from './workers/this.js';
import { type ValidWorker } from './workers/valid.js';
...
const countBy = (list: string[]) =>
  list.reduce((acc: Record<string, number>, key: string) => {
    acc[key] = (acc[key] ?? 0) + 1;
    return acc;
  }, {});
...
describe('Basic Features', () => {
  ...
  beforeEach(async () => {
    worker = await createThreadPool<typeof BasicWorker>(
      './__tests__/workers/basic',
      {
        size: 2,
        workerOptions: {
          workerData: {
  ...
  it('throws error if method is not available on worker', async () => {
    try {
      ...
      expect(err).toHaveProperty(
        'message',
        '"notWorkerMethod" is not a function in this worker thread'
      );
    }
  });
  ...
  it('can call a method on all workers', async () => {
    ...
    expect([value1, value2]).toEqual(['got value 14', 'got value 15']);
  });
  ...
  it('exposes the pool size as readonly', () => {
    ...
    expect(() => {
      ...
      'Cannot set property size of [object Object] which has only a getter'
    );
...
describe('Refill', () => {
  it('can automatically refill the pool when workers unexpectedly exit', async () => {
    const worker = await createThreadPool<typeof BasicWorker>(
      './__tests__/workers/basic',
      {
        size: 2,
        autoRefill: true,
...
describe('Modules', () => {
  it('handles a module without default export', async () => {
    const worker = await createThreadPool<ValidWorkerModule>(
      './__tests__/workers/module',
      {
        size: 2,
      }
    );
...
if (majorVersion >= 13) {
  describe('ES6 Modules', () => {
    ...
    it.skip('can expose methods from worker module', async () => {
      worker = await createThreadPool(
        path.resolve(__dirname, './__tests__/workers/es6-module.mjs'),
        {
          size: 2,
        }
      );
    ...
    it.skip('treats only default export as worker module', async () => {
      worker = await createThreadPool(
        path.resolve(__dirname, './__tests__/workers/es6-default.mjs'),
        {
          size: 2,
        }
      );
...
describe('Error Handling', () => {
  it('forwards worker method errors with worker stack trace', async () => {
    ...
    try {
      ...
      expect(err).toHaveProperty(
        'message',
        'worker triggered this error message'
      );
      expect(err).toHaveProperty(
        'stack',
        expect.stringContaining('workers/basic.ts')
      );
    }
    worker.pool.terminate();
  });
  ...
  it('rejects open method calls when a worker crashes', async () => {
    ...
    expect(mapBy).toEqual({
      ...
      'All workers exited before resolving (use an error event handler or DEBUG=armit-worker:*)': 2,
    });
  ...
  it('rejects open method calls when worker exits', async () => {
    ...
    result.map((err) =>
      expect(err).toHaveProperty(
        'message',
        'Worker thread exited before resolving'
      )
    );
    worker.pool.terminate();
  });
  ...
  it('rejects waiting method calls when all workers exited', async () => {
    ...
    expect(one).toHaveProperty(
      'message',
      'Worker thread exited before resolving'
    );
    expect(two).toHaveProperty(
      'message',
      'Worker thread exited before resolving'
    );
    expect(three).toHaveProperty(
      'message',
      'All workers exited before resolving (use an error event handler or DEBUG=puarmit-workerddle:*)'
    );
  ...
  it('emits an error event when a worker errors', async () => {
    ...
    expect(fn.mock.calls[0][0]).toHaveProperty(
      'stack',
      expect.stringContaining('workers/basic.ts')
    );
    worker.pool.terminate();
...
describe('Startup', () => {
  it('terminates armit-worker when workers fail in startup phase', async () => {
    ...
    expect(startupError).toHaveProperty(
      'message',
      'Failing before even exporting any method'
    );
  });
  ...
  it('rejects modules not exporting an object', async () => {
    ...
    expect(startupError).toHaveProperty(
      'message',
      'Worker should export an object, got null'
    );
  });
});
...
describe('Chaining', () => {
  it('returns the proxy when a worker object returns itself', async () => {
    const worker = await createThreadPool<ChainWorkerClass>(
      './__tests__/workers/this'
    );
...
describe('Callbacks', () => {
  ...
  it('handles an exposed event emitter', async () => {
    const worker = await createThreadPool<WorkerWithEmitter>(
      './__tests__/workers/eventemitter'
    );
