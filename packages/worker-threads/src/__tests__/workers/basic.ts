...
export default {
  ...
  asyncFn: async (arg1, timeout = 10) =>
    new Promise((resolve) =>
      setTimeout(() => resolve('got async ' + arg1), timeout)
    ),
  ...
  throwIfCalledAtTheSameTime: async (timeout) => {
    if (currentlyHandling) {
      throw new Error(
        'Should not be called while another method call is handled'
      );
    }
