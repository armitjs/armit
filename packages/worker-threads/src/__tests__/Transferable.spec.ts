import { createThreadPool, withTransfer } from '../index.js';
...
describe('Transferable', () => {
  ...
  it('can transfer a return value from a worker', async () => {
    ...
    if (majorVersion < 13) {
      expect(err).toHaveProperty(
        'message',
        'Cannot perform %TypedArray%.prototype.map on a neutered ArrayBuffer'
      );
    } else {
      expect(err).toHaveProperty(
        'message',
        'Cannot perform %TypedArray%.prototype.map on a detached ArrayBuffer'
      );
    }
  });
  ...
  it('handles error when a neutered array is accessed/transferred again from worker', async () => {
    ...
    try {
      ...
      expect(err.message).toMatch(
        'The ArrayBuffer for transfer is already detached'
      );
    }
  });
  ...
  it('can transfer a buffer to worker, manipulate it and transfer it back', async () => {
    const uint8Array = new Uint8Array([1, 2, 3, 4]);
    const result = await worker.manipulateAndTransfer(
      withTransfer(uint8Array, [uint8Array])
    );
  ...
  it('can transfer nested values', async () => {
    const uint8Array = new Uint8Array([1, 2, 3, 4]);
    const result = await worker.transferNested(
      withTransfer(
        {
          value: uint8Array,
        },
