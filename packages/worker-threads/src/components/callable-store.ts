...
import {
  ...
  type CallMessage,
  isThreadCallbackMessage,
  isThreadErrorMessage,
  isThreadFreeFunctionMessage,
  isThreadFunctionMessage,
  MainMessageAction,
...
export class CallableStore extends EventEmitter {
  ...
  createCallMessage(
    ...
    args: any[],
    callable: Callable
  ): { msg: CallMessage; transferables: Transferable[] } {
    ...
    if (isThreadCallbackMessage(msg)) {
      ...
      if (fnHolder) {
        ...
        Promise.resolve()
          .then(() => mFn(...msg.args))
          .catch((err) =>
            this.emit('callback:error', err, id, msg.key, msg.pos)
          );
      }
      return true;
